...
import { Sparkles, Loader2, RotateCcw, Timer } from "lucide-react";
...
export const CreationView: React.FC = () => {
  const { language, provider, autoGenerate, setAutoGenerate } =
    useSettingsStore();
  ...
  const AUTO_GENERATE_INTERVAL = 60; // seconds

  // Keep the latest handleGenerate in a ref so the interval always calls the
  // current implementation without restarting the timer on every render.
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      setAutoCountdown((prev) => {
        if (prev <= 1) {
          // Time to fire an automatic generation (if the app is idle).
  ...
  const handleToggleAutoGenerate = () => {
    ...
    setAutoCountdown(AUTO_GENERATE_INTERVAL);
    if (next) toast.info(t.autoGenerateOn);
  };
      ...
      <aside className="w-full md:max-w-sm flex-shrink-0 flex flex-col gap-4 md:gap-6">
        <div className="flex-grow space-y-4 md:space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Tooltip
              content={autoGenerate ? t.autoGenerateEnabled : t.autoGenerateDisabled}
            >
              <button
                type="button"
                onClick={handleToggleAutoGenerate}
                className={`flex-shrink-0 flex items-center justify-center gap-1.5 h-12 min-w-[44px] px-2 rounded-xl border transition-all shadow-lg active:scale-95 ${
                  autoGenerate
